...
:::tip
关于 ChatClient 中命令式和响应式编程模型组合使用的相关说明，请参阅本文档底部的**实现说明**部分。
:::
...
此时，您需要通过设置属性 `spring.ai.chat.client.enabled=false` 来禁用`ChatClient.Builder` 自动配置。这允许您手动创建多个 ChatClient 实例。
