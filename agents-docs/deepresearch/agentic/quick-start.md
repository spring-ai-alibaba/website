---
title: DeepResearch(Agentic)快速开始
...
A DeepResearch agent built with **Spring AI Alibaba Agentic Framework** that conducts thorough research and writes polished reports. Using an LLM to call tools in a loop is the simplest form of an agent, but this can yield agents that are "shallow". DeepResearch implements **task planning**, **sub agents**, **file system access**, and a **detailed prompt** to solve complex, multi-step research tasks.
...
![Architecture](/img/deepresearch/agent-chat-ui.jpg)
...
![Architecture](/img/deepresearch/architecture.png)
