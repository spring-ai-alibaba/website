...
流式 API 提供了用于构建传递给 AI model 作为输入的 [Prompt](api/prompt.adoc#_prompt) 的各个组成部分的方法。
...
您可以为任何 [ChatModel](api/chatmodel.adoc) Spring Boot 自动配置获取自动配置的 `ChatClient.Builder` 实例，或以编程方式创建一个。
...
来自 AI model 的响应是由类型 [ChatResponse](api/chatmodel.adoc#ChatResponse) 定义的丰富结构。
它包括有关如何生成响应的元数据，还可以包含多个响应，称为 [Generation](api/chatmodel.adoc#Generation)s，每个都有自己的元数据。
...
同时，您应该使用 [Structured Output Converter](api/structured-output-converter.adoc#StructuredOutputConverter) 显式转换聚合响应，如下所示。
...
> 它*不会*影响 [Advisors](api/retrieval-augmented-generation.adoc#_questionansweradvisor)（如 `QuestionAnswerAdvisor`）内部使用的模板，它们有自己的模板自定义机制（请参阅 [自定义 Advisor 模板](api/retrieval-augmented-generation.adoc#_custom_template)）。
...
[Advisors API](api/advisors.adoc) 提供了一种灵活而强大的方式来拦截、修改和增强 Spring 应用程序中的 AI 驱动交互。
...
[了解 Question Answer Advisor](api/retrieval-augmented-generation.adoc#_questionansweradvisor)

### Retrieval Augmented Generation

请参阅 [Retrieval Augmented Generation](api/retrieval-augmented-generation.adoc) 指南。
...
有关更多详细信息和使用示例，请参阅 [Chat Memory](api/chat-memory.adoc) 文档。
