...
| Property | Description |
| --- | --- |
| spring.ai.azure.openai.embedding.enabled (Removed and no longer valid) | Enable Azure OpenAI embedding model. |
| spring.ai.model.embedding | Enable Azure OpenAI embedding model. |
| spring.ai.azure.openai.embedding.metadata-mode | Document content extraction mode |
| spring.ai.azure.openai.embedding.options.deployment-name | This is the value of the 'Deployment Name' as presented in the Azure AI Portal |
...
TIP: All properties prefixed with `spring.ai.azure.openai.embedding.options` can be overridden at runtime by adding a request specific [embedding-options](#embedding-options) to the `EmbeddingRequest` call.
