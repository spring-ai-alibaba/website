...
3. 用于计算文档嵌入的 `EmbeddingModel` 实例。有多个选项可用：
   - 如果需要，为 [EmbeddingModel](api/embeddings#available-implementations) 提供一个 API key，用于生成 `OceanBaseVectorStore` 存储的嵌入。
...
您可以将通用的、可移植的 [metadata filters](api/vectordbs#_metadata_filters) 与 OceanBase 存储一起使用。
