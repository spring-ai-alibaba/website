...
| `spring.ai.vectorstore.oracle.forced-normalization` | 允许在插入和相似性搜索之前启用向量归一化（如果为 true）。<br><br>警告：将此设置为 true 是允许 [search request similarity threshold](api/vectordbs#api-overview) 的要求。<br><br>注意：如果向量已归一化，您可以使用 `DOT` 或 `COSINE` 以获得最佳性能。 | false |
