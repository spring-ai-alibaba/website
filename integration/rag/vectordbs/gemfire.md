...
2. 用于计算文档嵌入的 `EmbeddingModel` bean。请参阅 [EmbeddingModel](api/embeddings#available-implementations) 部分了解更多信息。
在您的机器上本地运行的选项是 [ONNX](api/embeddings/onnx) 和 all-MiniLM-L6-v2 Sentence Transformers。
...
您可以将通用的、可移植的 [metadata filters](api/vectordbs#metadata-filters) 与 GemFire VectorStore 一起使用。
