...
MCP Server Security 模块为 [Spring AI's MCP servers](api/mcp/mcp-server-boot-starter-docs.adoc) 提供 OAuth 2.0 资源服务器功能。
...
* 不支持已弃用的 SSE 传输。使用 [Streamable HTTP](api/mcp/mcp-streamable-http-server-boot-starter-docs.adoc) 或 [stateless transport](api/mcp/mcp-stateless-server-boot-starter-docs.adoc)。
* 不支持基于 WebFlux 的服务器。
* 不支持 Opaque tokens。使用 JWT。

## MCP Client Security

MCP Client Security 模块为 [Spring AI's MCP clients](api/mcp/mcp-client-boot-starter-docs.adoc) 提供 OAuth 2.0 支持，支持基于 HttpClient 的客户端（来自 `spring-ai-starter-mcp-client`）和基于 WebClient 的客户端（来自 `spring-ai-starter-mcp-client-webflux`）。
