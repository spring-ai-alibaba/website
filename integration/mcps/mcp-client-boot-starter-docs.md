...
MCP Client Boot Starter 通过 `ToolContextToMcpMetaConverter` 接口支持将 Spring AI 的 [ToolContext](api/tools.adoc#_tool_context) 转换为 MCP 工具调用元数据的可自定义转换。
...
有关所有可用注解及其使用模式的详细信息，请参阅 [MCP Client Annotations](api/mcp/mcp-annotations-client.adoc) 文档。
