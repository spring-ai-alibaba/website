...
> **注意：** 此实现使用来自 OpenAI 的官方 [OpenAI Java SDK](https://github.com/openai/openai-java)。对于替代的 Spring AI 实现，请参阅 [OpenAI Chat](api/chat/openai-chat)。
...
您可以利用现有的 [BeanOutputConverter](api/structured-output-converter#_bean_output_converter) 工具：
...
此实现与 [Spring AI OpenAI](api/chat/openai-chat) 实现在几个方面有所不同：
...
这些功能在 [Spring AI OpenAI](api/chat/openai-chat) 实现中可用。
